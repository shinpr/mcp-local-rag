...
该工具会立即返回 `jobId`。客户端应持续查询 `sync_status`，直到状态变为 `succeeded` 或 `failed`。发生变化的 PDF 会按已为其记录的视觉模式重新导入：最初带说明文字索引的 PDF 会保留说明文字，没有记录视觉模式的 PDF 仍按纯文本处理；`sync_start` 没有更改视觉模式的选项。
