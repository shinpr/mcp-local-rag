...
| `RAG_RERANK_CMD` | 未设置 | 仅 MCP 服务器：用于重排结果的外部命令模板。命中文本经标准输入发送；`{query}` 用于传入查询。 |
...
服务器通过标准输入把每批搜索结果（包括命中片段的文本）交给命令。只有模板包含 `{query}` 时，查询才会作为参数传入。调用远程服务的命令可能把收到的内容发送到本机之外。

填写可执行文件及完整的参数模板，在命令需要查询和结果数量的位置使用 `{query}` 和 `{top}`；服务器不会自动追加参数。单引号或双引号可将含空格的路径或参数组合在一起，反斜杠按字面保留。服务器不经过 shell 启动命令，因此 Windows 上由 npm 安装的 `.cmd` 包装脚本无法启动。
...
如果现有配置依赖服务器自动追加 `--query` 和 `--top`，请像上面的示例一样将它们写入模板。
